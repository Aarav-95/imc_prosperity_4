...
def _z_take_orders(state, cfg, store):
    """Effective mean uses an Exponential Moving Average (EMA) with alpha mapped
    directly from the stated prior (alpha = 1 / prior). Deep ITM options with
    1 billion prior will have alpha ≈ 0, keeping them perfectly static.
    State is persisted in `store` under key _ema[sym] = ema_value.
    """
    sym = cfg["symbol"]
    depth = state.order_depths.get(sym)
    if not depth or not depth.buy_orders or not depth.sell_orders:
    ...
    zt = store.setdefault("_ema", {})
    static_mean = float(cfg["mean"])
    
    # Emulate the prior logic with EMA alpha = 1 / prior
    # A prior of 10**9 results in alpha = ~0.0 (static mean).
